# 完整的Streamlit应用更新脚本（包含增强训练功能）
# 直接执行此脚本即可更新material_generator_app.py
...
st.set_page_config(page_title="无人机素材多角度生成系统", page_icon="🚁", layout="wide")
...
with st.sidebar:
    ...
    if enable_enhancement:
        target_score = st.slider("目标质量分数", 80, 100, 90, 1)
        ...
        if st.session_state.enhancement_results:
            ...
            for result in enhancement_result['results']:
                if result.get('success', False):
                    enhancement_history_data.append({
                        ...
                        '初始分数': f"{result.get('enhancement_history', [{}])[0].get('score', 0):.2f}%" if result.get('enhancement_history') else "N/A",
                        '最终分数': f"{result.get('final_score', 0):.2f}%",
                        '提升幅度': f"+{result.get('improvement', 0):.2f}%",
                        '迭代次数': result.get('iterations', 0),
                        '是否达标': "✅" if result.get('target_achieved', False) else "❌"
                    })
            ...
            if enhancement_history_data:
                ...
                fig.add_trace(go.Bar(
                    ...
                    y=[float(item['提升幅度'].lstrip('+').rstrip('%')) for item in enhancement_history_data],
...
Write-Host "`n✅ Streamlit应用更新完成！" -ForegroundColor Green
Write-Host "`n现在可以启动Streamlit应用：" -ForegroundColor Cyan
