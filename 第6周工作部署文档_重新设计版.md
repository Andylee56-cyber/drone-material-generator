# 第6周工作部署文档 - 无人机素材多角度生成与分析系统
## 重新设计版 - 单图输入→多角度生成→8维雷达图分析

> **核心功能**: 输入一张图片，系统自动生成多角度素材，并生成8维度雷达图可视化

---

## 📋 系统功能说明

### 核心功能流程
```
输入单张图片 
  ↓
自动生成多角度素材（旋转、翻转、亮度、对比度等变换）
  ↓
8维度质量分析（图片数据量、光照质量、目标尺寸等）
  ↓
生成8维度雷达图（圆形可视化图表）
  ↓
输出分析报告
```

### 8个分析维度
1. **图片数据量** - 基于分辨率和文件大小
2. **拍摄光照质量** - 基于亮度、对比度、曝光
3. **目标尺寸** - 基于YOLO检测的目标平均尺寸
4. **目标完整性** - 基于目标是否被裁剪或遮挡
5. **数据均衡度** - 基于不同类别目标的分布均衡性
6. **产品丰富度** - 基于检测到的目标类别数量
7. **目标密集度** - 基于单位面积内的目标数量
8. **场景复杂度** - 基于背景复杂度、纹理丰富度

---

## 📁 第一步：建立完整的目录结构

### 1.1 打开PowerShell并定位到项目目录

**执行命令**:

```powershell
# 进入项目根目录
cd D:\mlflow_learning_project

# 验证当前位置
pwd
```

**预期输出**:
```
Path
----
D:\mlflow_learning_project
```

### 1.2 创建所有必需的目录结构

**执行命令**:

```powershell
# 创建所有必需目录
New-Item -ItemType Directory -Path "agents","app\web","scripts","reports","temp_uploads","test_images","generated_materials" -Force | Out-Null

# 验证目录创建
Get-ChildItem -Directory | Select-Object Name | Format-Table -AutoSize
```

**预期输出**:
```
Name
----
agents
app
generated_materials
reports
scripts
temp_uploads
test_images
```

---

## 🔧 第二步：环境配置与依赖安装

### 2.1 激活Conda环境

**执行命令**:

```powershell
# 激活环境
conda activate uav_adv

# 验证环境（命令提示符前应显示 (uav_adv)）
python --version
```

### 2.2 安装依赖包

**执行命令**:

```powershell
# 安装核心依赖
pip install ultralytics==8.2.74
pip install plotly>=5.23.0
pip install streamlit>=1.37.1
pip install opencv-python-headless==4.9.0.80
pip install pillow>=9.5.0
pip install numpy>=1.23.5
pip install pandas>=2.0.3

# 验证安装
python -c "import ultralytics, plotly, streamlit, cv2, PIL; print('✅ 所有依赖OK')"
```

---

## 🧱 第三步：创建/更新核心 Python 文件（在 PowerShell 中创建）

> **说明**：以下命令必须在 PowerShell 中执行，确保每个 `.py` 文件都在本地创建，便于自行管理。

### 3.1 创建 `agents\image_multi_angle_generator.py`

```powershell
@'
""" 
图片多角度生成器 
Image Multi-Angle Generator 
从单张输入图片生成多角度的素材图片 
"""

import numpy as np
import cv2
from PIL import Image
from pathlib import Path
from typing import List, Dict, Optional
import random
from datetime import datetime


class ImageMultiAngleGenerator:
    """图片多角度生成器"""

    def __init__(self):
        """初始化生成器"""
        self.supported_formats = {'.jpg', '.jpeg', '.png', '.bmp'}

    def generate_multi_angle_images(
        self,
        input_image_path: str,
        output_dir: str,
        num_generations: int = 8,
        transformations: List[str] = None
    ) -> Dict:
        """
        从单张图片生成多角度素材

        参数:
            input_image_path: 输入图片路径
            output_dir: 输出目录
            num_generations: 生成数量
            transformations: 变换类型列表（None表示使用所有变换）

        返回:
            生成结果字典
        """
        input_path = Path(input_image_path)
        output_path = Path(output_dir)
        output_path.mkdir(parents=True, exist_ok=True)

        if not input_path.exists():
            raise FileNotFoundError(f"输入图片不存在: {input_image_path}")

        # 读取原始图片
        img = cv2.imread(str(input_path))
        if img is None:
            pil_img = Image.open(input_path)
            img = cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)

        h, w = img.shape[:2]

        if transformations is None:
            transformations = [
                'original', 'rotate_15', 'rotate_-15', 'rotate_30',
                'flip_horizontal', 'flip_vertical',
                'brightness_up', 'brightness_down',
                'contrast_up', 'contrast_down',
                'saturation_up', 'saturation_down',
                'blur_slight', 'sharp',
                'crop_center', 'crop_corner',
                'zoom_in', 'zoom_out',
                'perspective', 'noise'
            ]

        if num_generations > len(transformations):
            selected_transforms = transformations + random.sample(
                transformations,
                num_generations - len(transformations)
            )
        else:
            selected_transforms = random.sample(transformations, num_generations)

        generated_files = []
        metadata = []

        for idx, transform_type in enumerate(selected_transforms, 1):
            try:
                transformed_img = self._apply_transformation(img, transform_type, h, w)
                output_filename = f"generated_{idx:03d}_{transform_type}.jpg"
                output_file = output_path / output_filename
                cv2.imwrite(str(output_file), transformed_img, [cv2.IMWRITE_JPEG_QUALITY, 95])

                generated_files.append(str(output_file))
                metadata.append({
                    'index': idx,
                    'original_path': str(input_path),
                    'generated_path': str(output_file),
                    'transformation': transform_type,
                    'filename': output_filename
                })
            except Exception as e:
                print(f"⚠️ 生成失败 {transform_type}: {e}")
                continue

        metadata_file = output_path / f"generation_metadata_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        import json
        with open(metadata_file, 'w', encoding='utf-8') as f:
            json.dump({
                'generation_time': datetime.now().isoformat(),
                'original_image': str(input_path),
                'output_dir': str(output_path),
                'num_requested': num_generations,
                'num_generated': len(generated_files),
                'generated_images': metadata
            }, f, ensure_ascii=False, indent=2)

        return {
            'success': True,
            'original_image': str(input_path),
            'output_dir': str(output_path),
            'num_generated': len(generated_files),
            'generated_files': generated_files,
            'metadata_file': str(metadata_file)
        }

    def _apply_transformation(self, img: np.ndarray, transform_type: str, h: int, w: int) -> np.ndarray:
        """应用指定的变换"""
        result = img.copy()

        if transform_type == 'original':
            return result
        elif transform_type == 'rotate_15':
            center = (w // 2, h // 2)
            M = cv2.getRotationMatrix2D(center, 15, 1.0)
            result = cv2.warpAffine(result, M, (w, h), borderValue=(128, 128, 128))
        elif transform_type == 'rotate_-15':
            center = (w // 2, h // 2)
            M = cv2.getRotationMatrix2D(center, -15, 1.0)
            result = cv2.warpAffine(result, M, (w, h), borderValue=(128, 128, 128))
        elif transform_type == 'rotate_30':
            center = (w // 2, h // 2)
            M = cv2.getRotationMatrix2D(center, 30, 1.0)
            result = cv2.warpAffine(result, M, (w, h), borderValue=(128, 128, 128))
        elif transform_type == 'flip_horizontal':
            result = cv2.flip(result, 1)
        elif transform_type == 'flip_vertical':
            result = cv2.flip(result, 0)
        elif transform_type == 'brightness_up':
            result = cv2.convertScaleAbs(result, alpha=1.2, beta=20)
        elif transform_type == 'brightness_down':
            result = cv2.convertScaleAbs(result, alpha=0.8, beta=-20)
        elif transform_type == 'contrast_up':
            result = cv2.convertScaleAbs(result, alpha=1.3, beta=0)
        elif transform_type == 'contrast_down':
            result = cv2.convertScaleAbs(result, alpha=0.7, beta=0)
        elif transform_type == 'saturation_up':
            hsv = cv2.cvtColor(result, cv2.COLOR_BGR2HSV)
            hsv[:, :, 1] = cv2.multiply(hsv[:, :, 1], 1.3)
            result = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
        elif transform_type == 'saturation_down':
            hsv = cv2.cvtColor(result, cv2.COLOR_BGR2HSV)
            hsv[:, :, 1] = cv2.multiply(hsv[:, :, 1], 0.7)
            result = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
        elif transform_type == 'blur_slight':
            result = cv2.GaussianBlur(result, (5, 5), 0)
        elif transform_type == 'sharp':
            kernel = np.array([[-1, -1, -1], [-1,  9, -1], [-1, -1, -1]])
            result = cv2.filter2D(result, -1, kernel)
        elif transform_type == 'crop_center':
            crop_size = min(h, w) * 0.8
            x = int((w - crop_size) / 2)
            y = int((h - crop_size) / 2)
            result = result[y:int(y+crop_size), x:int(x+crop_size)]
            result = cv2.resize(result, (w, h))
        elif transform_type == 'crop_corner':
            crop_size = min(h, w) * 0.8
            result = result[0:int(crop_size), 0:int(crop_size)]
            result = cv2.resize(result, (w, h))
        elif transform_type == 'zoom_in':
            M = cv2.getRotationMatrix2D((w/2, h/2), 0, 1.2)
            result = cv2.warpAffine(result, M, (w, h))
        elif transform_type == 'zoom_out':
            M = cv2.getRotationMatrix2D((w/2, h/2), 0, 0.8)
            result = cv2.warpAffine(result, M, (w, h), borderValue=(128, 128, 128))
        elif transform_type == 'perspective':
            pts1 = np.float32([[0, 0], [w, 0], [0, h], [w, h]])
            pts2 = np.float32([[w*0.1, h*0.1], [w*0.9, h*0.1], [0, h], [w, h]])
            M = cv2.getPerspectiveTransform(pts1, pts2)
            result = cv2.warpPerspective(result, M, (w, h))
        elif transform_type == 'noise':
            noise = np.random.randint(0, 25, result.shape, dtype=np.uint8)
            result = cv2.add(result, noise)

        return result


if __name__ == "__main__":
    generator = ImageMultiAngleGenerator()
    result = generator.generate_multi_angle_images(
        input_image_path="test.jpg",
        output_dir="generated",
        num_generations=8
    )
    print(f"生成了 {result['num_generated']} 张图片")
'@ | Set-Content "agents\image_multi_angle_generator.py" -Encoding UTF8
```

### 3.2 创建 `scripts\generate_from_single_image.py`

```powershell
@'
"""
从单张图片生成多角度素材并分析
Generate Multi-Angle Materials from Single Image and Analyze
"""

import argparse
import json
from pathlib import Path
import sys

project_root = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(project_root))

from agents.image_multi_angle_generator import ImageMultiAngleGenerator
from agents.image_quality_analyzer import ImageQualityAnalyzer
from agents.material_generator_agent import MaterialGeneratorAgent


def main():
    parser = argparse.ArgumentParser(description="从单张图片生成多角度素材并分析")
    parser.add_argument("--input-image", type=str, required=True, help="输入图片路径")
    parser.add_argument("--output-dir", type=str, default="generated_materials", help="输出目录")
    parser.add_argument("--num-generations", type=int, default=8, help="生成图片数量")
    parser.add_argument("--analyze", action="store_true", help="生成后自动分析")
    parser.add_argument("--yolo-model", type=str, default=None, help="YOLO模型路径（可选）")
    args = parser.parse_args()

    input_path = Path(args.input_image)
    if not input_path.exists():
        print(f"❌ 输入图片不存在: {args.input_image}")
        return

    print("=" * 60)
    print("从单张图片生成多角度素材")
    print("=" * 60)
    print(f"输入图片: {args.input_image}")
    print(f"输出目录: {args.output_dir}")
    print(f"生成数量: {args.num_generations}")
    print("=" * 60)

    print("\n步骤1: 生成多角度图片...")
    generator = ImageMultiAngleGenerator()
    try:
        result = generator.generate_multi_angle_images(
            input_image_path=str(input_path),
            output_dir=args.output_dir,
            num_generations=args.num_generations
        )
        if result['success']:
            print(f"✅ 成功生成 {result['num_generated']} 张图片")
## 📝 第三步：验证核心文件

### 3.1 检查核心文件是否存在

**执行命令**:

```powershell
# 检查文件
$files = @(
    "agents\__init__.py",
    "agents\image_quality_analyzer.py",
    "agents\image_multi_angle_generator.py",
    "agents\material_generator_agent.py",
    "app\web\material_generator_app.py",
    "scripts\generate_from_single_image.py"
)

foreach ($file in $files) {
    if (Test-Path $file) {
        Write-Host "✅ $file" -ForegroundColor Green
    } else {
        Write-Host "❌ 缺失: $file" -ForegroundColor Red
    }
}
```

### 3.2 验证模块导入

**执行命令**:

```powershell
python -c "
import sys
sys.path.insert(0, '.')
from agents.image_multi_angle_generator import ImageMultiAngleGenerator
from agents.image_quality_analyzer import ImageQualityAnalyzer
from agents.material_generator_agent import MaterialGeneratorAgent
print('✅ 所有模块导入成功')
"
```

---

## 🧪 第四步：测试多角度生成功能

### 4.1 准备测试图片

**执行命令**:

```powershell
# 方法1: 从指定目录复制

  $sourceImage = Get-ChildItem "D:\网上素材图片\*.jpg" | Select-Object -First 1
   if ($sourceImage) {
       Copy-Item "D:\网上素材图片\7010.jpg_wh860" -Destination "test_images\test_input.jpg" -Force
       Write-Host "✅ 已复制测试图片: $($sourceImage.Name)" -ForegroundColor Green
   }
# 方法2: 打开目录手动复制
explorer test_images

# 验证图片存在
if (Test-Path "test_images\test_input.jpg") {
    $img = Get-Item "test_images\test_input.jpg"
    Write-Host "✅ 测试图片: $($img.Name) ($([math]::Round($img.Length/1KB, 2)) KB)" -ForegroundColor Green
} else {
    Write-Host "❌ 请先准备测试图片" -ForegroundColor Red
}
```

### 4.2 测试多角度生成

**执行命令**:

```powershell
# 测试生成8张多角度图片
python -c "
import sys
sys.path.insert(0, '.')
from agents.image_multi_angle_generator import ImageMultiAngleGenerator

generator = ImageMultiAngleGenerator()
result = generator.generate_multi_angle_images(
    input_image_path='test_images/test_input.jpg',
    output_dir='test_generated',
    num_generations=8
)

print('=' * 60)
print('多角度生成测试结果')
print('=' * 60)
print(f'输入图片: test_images/test_input.jpg')
print(f'生成数量: {result[\"num_generated\"]} 张')
print(f'输出目录: {result[\"output_dir\"]}')
print('=' * 60)
print('✅ 生成测试完成')
"
```

**预期输出**:
```
============================================================
多角度生成测试结果
============================================================
输入图片: test_images/test_input.jpg
生成数量: 8 张
输出目录: test_generated
============================================================
✅ 生成测试完成
```

### 4.3 查看生成的图片

**执行命令**:

```powershell
# 查看生成的图片
if (Test-Path "test_generated") {
    $generated = Get-ChildItem "test_generated\generated_*.jpg"
    Write-Host "`n生成的图片:" -ForegroundColor Cyan
    $generated | ForEach-Object {
        Write-Host "  - $($_.Name) ($([math]::Round($_.Length/1KB, 2)) KB)" -ForegroundColor Green
    }
    
    # 打开目录查看
    explorer test_generated
}
```

---

## 🚀 第五步：测试完整流程（生成+分析）

### 5.1 执行完整流程测试

**执行命令**:

```powershell
# 使用脚本执行完整流程
python scripts\generate_from_single_image.py `
  --input-image "test_images\test_input.jpg" `
  --output-dir "test_complete" `
  --num-generations 8 `
  --analyze
```

**预期输出**:
```
============================================================
从单张图片生成多角度素材并分析
============================================================
输入图片: test_images\test_input.jpg
输出目录: test_complete
生成数量: 8
============================================================

步骤1: 生成多角度图片...
✅ 成功生成 8 张图片
输出目录: test_complete

步骤2: 分析生成的图片...
正在初始化分析器...
正在分析图片 1/8...
...
✅ 分析完成，结果保存到: test_complete\analysis_result_xxx.json
平均质量: 82.35%

✅ 处理完成！
```

### 5.2 验证分析结果

**执行命令**:

```powershell
# 查看分析结果
python -c "
import json
from pathlib import Path

result_file = list(Path('test_complete').glob('analysis_result_*.json'))[0]
with open(result_file, 'r', encoding='utf-8') as f:
    data = json.load(f)

print('=' * 60)
print('分析结果验证')
print('=' * 60)
print(f'分析图片数: {data[\"analysis\"][\"total_images\"]}')
print(f'整体质量: {data[\"recommendations\"][\"overall_quality\"]:.2f}%')
print('\n平均维度得分:')
for dim, score in data['analysis']['average_scores'].items():
    print(f'  {dim:20s}: {score:6.2f}%')
print('=' * 60)
"
```

---

## 🌐 第六步：启动Web界面

### 6.1 启动Web应用

**执行命令**:

```powershell
# 启动Streamlit应用
conda activate uav_adv
streamlit run app\web\material_generator_app.py --server.port 8502 --server.headless true

# 自动打开浏览器
start http://localhost:8502
```

**预期输出**:
```
  You can now view your Streamlit app in your browser.

  Local URL: http://loc lhost:8502
  Network URL: http://192.168.x.x:8502
```

### 6.2 使用Web界面

**操作步骤**:

1. **上传图片**:
   - 在浏览器中点击"上传一张无人机图片"
   - 选择你的测试图片（如 `test_images\test_input.jpg`）

2. **设置参数**:
   - 在侧边栏设置"生成图片数量"（默认8张）
   - 勾选"生成后自动分析"

3. **生成和分析**:
   - 点击"🚀 生成多角度素材并分析"按钮
   - 等待处理完成（约30-60秒）

4. **查看结果**:
   - 查看生成的多角度素材图片网格
   - 查看8维度雷达图（圆形图表）
   - 查看维度得分详情表格
   - 查看素材质量评估表格

---

## 📊 第七步：验证雷达图显示

### 7.1 检查雷达图是否正确显示

**在Web界面中验证**:

- [ ] 雷达图是圆形/多边形（不是条形图）
- [ ] 显示8个维度
- [ ] 每个维度都有数值（0-100%）
- [ ] 图表有颜色填充
- [ ] 显示图例和标题

### 7.2 验证数据准确性

**执行命令验证**:

```powershell
# 验证分析数据的完整性
python -c "
import sys
sys.path.insert(0, '.')
from agents.image_quality_analyzer import ImageQualityAnalyzer

analyzer = ImageQualityAnalyzer()
result = analyzer.analyze_single_image('test_images/test_input.jpg')

dimensions = ['图片数据量', '拍摄光照质量', '目标尺寸', '目标完整性', 
              '数据均衡度', '产品丰富度', '目标密集度', '场景复杂度']

print('=' * 60)
print('维度数据验证')
print('=' * 60)
for dim in dimensions:
    if dim in result:
        print(f'✅ {dim:20s}: {result[dim]:6.2f}%')
    else:
        print(f'❌ {dim:20s}: 缺失')
print('=' * 60)
"
```

---

## 🎯 完整工作流程总结

### 日常使用流程

1. **准备输入图片**:
   ```powershell
   # 将图片放到test_images目录
   Copy-Item "D:\你的图片\image.jpg" -Destination "test_images\input.jpg"
   ```

2. **启动Web界面**:
   ```powershell
   streamlit run app\web\material_generator_app.py --server.port 8502
   ```

3. **在Web界面操作**:
   - 上传图片
   - 设置生成数量
   - 点击生成按钮
   - 查看雷达图和分析结果

4. **查看生成结果**:
   ```powershell
   # 打开生成目录
   explorer generated_materials
   ```

### 命令行使用流程

```powershell
# 一键生成和分析
python scripts\generate_from_single_image.py `
  --input-image "test_images\input.jpg" `
  --output-dir "output" `
  --num-generations 8 `
  --analyze
```

---

## ✅ 验收检查清单

### 功能验收
- [ ] 能够上传单张图片
- [ ] 能够生成多角度素材（至少8张）
- [ ] 能够自动进行8维度分析
- [ ] 能够显示8维度雷达图（圆形图表）
- [ ] 雷达图显示所有8个维度
- [ ] 能够查看详细得分表格
- [ ] 能够查看质量评估表格

### 性能验收
- [ ] 生成8张图片时间 < 10秒
- [ ] 分析8张图片时间 < 60秒
- [ ] 雷达图加载时间 < 2秒

### 质量验收
- [ ] 生成的图片质量良好
- [ ] 雷达图美观清晰
- [ ] 数据准确可靠

---

## 🐛 常见问题

### Q1: 生成图片失败
**A**: 检查输入图片格式和路径是否正确
```powershell
# 检查图片
Get-Item "test_images\test_input.jpg"
```

### Q2: 雷达图不显示
**A**: 检查Plotly是否正确安装
```powershell
python -c "import plotly; print('Plotly版本:', plotly.__version__)"
```

### Q3: 分析速度慢
**A**: 确保使用GPU，或减少生成数量
```powershell
python -c "import torch; print('CUDA可用:', torch.cuda.is_available())"
```

---

**文档版本**: v4.0 (重新设计版 - 多角度生成+雷达图)  
**创建日期**: 2024年11月  
**最后更新**: 2024年11月

---

## 📎 附录：核心 Python 文件创建命令（PowerShell）

> **使用方法**：复制以下命令块到 PowerShell 执行，即可创建对应 `.py` 文件。确保当前目录为 `D:\mlflow_learning_project`。

### A1. 创建 `agents\image_multi_angle_generator.py`

```powershell
@'
"""
图片多角度生成器
Image Multi-Angle Generator
从单张输入图片生成多角度的素材图片
"""

import numpy as np
import cv2
from PIL import Image
from pathlib import Path
from typing import List, Dict, Optional
import random
from datetime import datetime


class ImageMultiAngleGenerator:
    """图片多角度生成器"""

    def __init__(self):
        """初始化生成器"""
        self.supported_formats = {'.jpg', '.jpeg', '.png', '.bmp'}

    def generate_multi_angle_images(
        self,
        input_image_path: str,
        output_dir: str,
        num_generations: int = 8,
        transformations: List[str] = None
    ) -> Dict:
        """
        从单张图片生成多角度素材
        """
        input_path = Path(input_image_path)
        output_path = Path(output_dir)
        output_path.mkdir(parents=True, exist_ok=True)

        if not input_path.exists():
            raise FileNotFoundError(f"输入图片不存在: {input_image_path}")

        img = cv2.imread(str(input_path))
        if img is None:
            pil_img = Image.open(input_path)
            img = cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)

        h, w = img.shape[:2]

        if transformations is None:
            transformations = [
                'original','rotate_15','rotate_-15','rotate_30',
                'flip_horizontal','flip_vertical',
                'brightness_up','brightness_down',
                'contrast_up','contrast_down',
                'saturation_up','saturation_down',
                'blur_slight','sharp',
                'crop_center','crop_corner',
                'zoom_in','zoom_out',
                'perspective','noise'
            ]

        if num_generations > len(transformations):
            selected_transforms = transformations + random.sample(
                transformations,
                num_generations - len(transformations)
            )
        else:
            selected_transforms = random.sample(transformations, num_generations)

        generated_files = []
        metadata = []

        for idx, transform_type in enumerate(selected_transforms, 1):
            try:
                transformed_img = self._apply_transformation(img, transform_type, h, w)
                output_filename = f"generated_{idx:03d}_{transform_type}.jpg"
                output_file = output_path / output_filename
                cv2.imwrite(str(output_file), transformed_img, [cv2.IMWRITE_JPEG_QUALITY, 95])

                generated_files.append(str(output_file))
                metadata.append({
                    'index': idx,
                    'original_path': str(input_path),
                    'generated_path': str(output_file),
                    'transformation': transform_type,
                    'filename': output_filename
                })
            except Exception as e:
                print(f"⚠️ 生成失败 {transform_type}: {e}")
                continue

        metadata_file = output_path / f"generation_metadata_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        import json
        with open(metadata_file, 'w', encoding='utf-8') as f:
            json.dump({
                'generation_time': datetime.now().isoformat(),
                'original_image': str(input_path),
                'output_dir': str(output_path),
                'num_requested': num_generations,
                'num_generated': len(generated_files),
                'generated_images': metadata
            }, f, ensure_ascii=False, indent=2)

        return {
            'success': True,
            'original_image': str(input_path),
            'output_dir': str(output_path),
            'num_generated': len(generated_files),
            'generated_files': generated_files,
            'metadata_file': str(metadata_file)
        }

    def _apply_transformation(self, img: np.ndarray, transform_type: str, h: int, w: int) -> np.ndarray:
        """应用指定的变换"""
        result = img.copy()
        if transform_type == 'original':
            return result
        elif transform_type == 'rotate_15':
            center = (w // 2, h // 2)
            M = cv2.getRotationMatrix2D(center, 15, 1.0)
            result = cv2.warpAffine(result, M, (w, h), borderValue=(128, 128, 128))
        elif transform_type == 'rotate_-15':
            center = (w // 2, h // 2)
            M = cv2.getRotationMatrix2D(center, -15, 1.0)
            result = cv2.warpAffine(result, M, (w, h), borderValue=(128, 128, 128))
        elif transform_type == 'rotate_30':
            center = (w // 2, h // 2)
            M = cv2.getRotationMatrix2D(center, 30, 1.0)
            result = cv2.warpAffine(result, M, (w, h), borderValue=(128, 128, 128))
        elif transform_type == 'flip_horizontal':
            result = cv2.flip(result, 1)
        elif transform_type == 'flip_vertical':
            result = cv2.flip(result, 0)
        elif transform_type == 'brightness_up':
            result = cv2.convertScaleAbs(result, alpha=1.2, beta=20)
        elif transform_type == 'brightness_down':
            result = cv2.convertScaleAbs(result, alpha=0.8, beta=-20)
        elif transform_type == 'contrast_up':
            result = cv2.convertScaleAbs(result, alpha=1.3, beta=0)
        elif transform_type == 'contrast_down':
            result = cv2.convertScaleAbs(result, alpha=0.7, beta=0)
        elif transform_type == 'saturation_up':
            hsv = cv2.cvtColor(result, cv2.COLOR_BGR2HSV)
            hsv[:, :, 1] = cv2.multiply(hsv[:, :, 1], 1.3)
            result = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
        elif transform_type == 'saturation_down':
            hsv = cv2.cvtColor(result, cv2.COLOR_BGR2HSV)
            hsv[:, :, 1] = cv2.multiply(hsv[:, :, 1], 0.7)
            result = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
        elif transform_type == 'blur_slight':
            result = cv2.GaussianBlur(result, (5, 5), 0)
        elif transform_type == 'sharp':
            kernel = np.array([[-1, -1, -1],[-1, 9, -1],[-1, -1, -1]])
            result = cv2.filter2D(result, -1, kernel)
        elif transform_type == 'crop_center':
            crop_size = min(h, w) * 0.8
            x = int((w - crop_size) / 2)
            y = int((h - crop_size) / 2)
            result = result[y:int(y+crop_size), x:int(x+crop_size)]
            result = cv2.resize(result, (w, h))
        elif transform_type == 'crop_corner':
            crop_size = min(h, w) * 0.8
            result = result[0:int(crop_size), 0:int(crop_size)]
            result = cv2.resize(result, (w, h))
        elif transform_type == 'zoom_in':
            M = cv2.getRotationMatrix2D((w/2, h/2), 0, 1.2)
            result = cv2.warpAffine(result, M, (w, h))
        elif transform_type == 'zoom_out':
            M = cv2.getRotationMatrix2D((w/2, h/2), 0, 0.8)
            result = cv2.warpAffine(result, M, (w, h), borderValue=(128, 128, 128))
        elif transform_type == 'perspective':
            pts1 = np.float32([[0, 0], [w, 0], [0, h], [w, h]])
            pts2 = np.float32([[w*0.1, h*0.1], [w*0.9, h*0.1], [0, h], [w, h]])
            M = cv2.getPerspectiveTransform(pts1, pts2)
            result = cv2.warpPerspective(result, M, (w, h))
        elif transform_type == 'noise':
            noise = np.random.randint(0, 25, result.shape, dtype=np.uint8)
            result = cv2.add(result, noise)

        return result


if __name__ == "__main__":
    generator = ImageMultiAngleGenerator()
    result = generator.generate_multi_angle_images(
        input_image_path="test_image.jpg",
        output_dir="generated",
        num_generations=8
    )
    print(f"生成了 {result['num_generated']} 张图片")
'@ | Set-Content "agents\image_multi_angle_generator.py" -Encoding UTF8
```

### A2. 创建 `scripts\generate_from_single_image.py`

```powershell
@'
"""
从单张图片生成多角度素材并分析
Generate Multi-Angle Materials from Single Image and Analyze
"""

import argparse
import json
from pathlib import Path
import sys

project_root = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(project_root))

from agents.image_multi_angle_generator import ImageMultiAngleGenerator
from agents.image_quality_analyzer import ImageQualityAnalyzer
from agents.material_generator_agent import MaterialGeneratorAgent


def main():
    parser = argparse.ArgumentParser(description="从单张图片生成多角度素材并分析")
    parser.add_argument("--input-image", type=str, required=True, help="输入图片路径")
    parser.add_argument("--output-dir", type=str, default="generated_materials", help="输出目录")
    parser.add_argument("--num-generations", type=int, default=8, help="生成图片数量")
    parser.add_argument("--analyze", action="store_true", help="生成后自动分析")
    parser.add_argument("--yolo-model", type=str, default=None, help="YOLO模型路径（可选）")
    args = parser.parse_args()

    input_path = Path(args.input_image)
    if not input_path.exists():
        print(f"❌ 输入图片不存在: {args.input_image}")
        return

    print("=" * 60)
    print("从单张图片生成多角度素材")
    print("=" * 60)
    print(f"输入图片: {args.input_image}")
    print(f"输出目录: {args.output_dir}")
    print(f"生成数量: {args.num_generations}")
    print("=" * 60)

    print("\n步骤1: 生成多角度图片...")
    generator = ImageMultiAngleGenerator()
    try:
        result = generator.generate_multi_angle_images(
            input_image_path=str(input_path),
            output_dir=args.output_dir,
            num_generations=args.num_generations
        )
        if result['success']:
            print(f"✅ 成功生成 {result['num_generated']} 张图片")
            print(f"输出目录: {result['output_dir']}")
        else:
            print("❌ 生成失败")
            return
    except Exception as e:
        print(f"❌ 生成过程出错: {e}")
        import traceback
        traceback.print_exc()
        return

    # 步骤2: 分析生成的图片（如果启用）
    if args.analyze:
        print("\n步骤2: 分析生成的图片...")
        analyzer = ImageQualityAnalyzer(yolo_model_path=args.yolo_model)
        agent = MaterialGeneratorAgent(yolo_model_path=args.yolo_model)
        try:
            # 分析所有生成的图片
            analysis_result = agent.analyze_and_evaluate(result['generated_files'])
            
            # 保存分析结果
            analysis_file = Path(args.output_dir) / f"analysis_result_{Path(result['metadata_file']).stem.split('_')[-1]}.json"
            with open(analysis_file, 'w', encoding='utf-8') as f:
                json.dump(analysis_result, f, ensure_ascii=False, indent=2)
            
            print(f"✅ 分析完成，结果保存到: {analysis_file}")
            print(f"平均质量: {analysis_result['recommendations']['overall_quality']:.2f}%")
            
        except Exception as e:
            print(f"⚠️ 分析过程出错: {e}")
            import traceback
            traceback.print_exc()

    print("\n✅ 处理完成！")


if __name__ == "__main__":
    main()
'@ | Set-Content "scripts\generate_from_single_image.py" -Encoding UTF8

# 验证文件创建成功（文件应该有120行左右）
Write-Host "`n验证文件创建..." -ForegroundColor Cyan
if (Test-Path "scripts\generate_from_single_image.py") {
    $lineCount = (Get-Content "scripts\generate_from_single_image.py" | Measure-Object -Line).Lines
    Write-Host "✅ 文件已创建，共 $lineCount 行" -ForegroundColor Green
    if ($lineCount -lt 100) {
        Write-Host "⚠️ 警告: 文件行数少于预期，可能不完整！" -ForegroundColor Yellow
    }
} else {
    Write-Host "❌ 文件创建失败！" -ForegroundColor Red
}
```

### A3. 创建 `app\web\material_generator_app.py`

```powershell
@'
"""
无人机素材多角度生成与分析系统 - Streamlit Web界面
"""

import streamlit as st
import plotly.graph_objects as go
import pandas as pd
import numpy as np
from pathlib import Path
import sys
from PIL import Image
import time

project_root = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(project_root))

from agents.image_multi_angle_generator import ImageMultiAngleGenerator
from agents.image_quality_analyzer import ImageQualityAnalyzer
from agents.material_generator_agent import MaterialGeneratorAgent

st.set_page_config(page_title="无人机素材多角度生成系统", page_icon="🚁", layout="wide")
st.title("🚁 无人机素材多角度生成与分析系统")
st.markdown("**功能**: 输入一张图片，自动生成多角度素材，并生成8维度雷达图分析")
st.markdown("---")

if 'generator' not in st.session_state:
    st.session_state.generator = ImageMultiAngleGenerator()
if 'agent' not in st.session_state:
    st.session_state.agent = MaterialGeneratorAgent()
if 'generated_images' not in st.session_state:
    st.session_state.generated_images = []
if 'analysis_results' not in st.session_state:
    st.session_state.analysis_results = None

with st.sidebar:
    st.header("⚙️ 系统配置")
    num_generations = st.slider("生成图片数量", 4, 20, 8, 1)
    auto_analyze = st.checkbox("生成后自动分析", value=True)
    st.markdown("---")
    st.markdown("### 📊 8个分析维度")
    for dim in [
        "1. 图片数据量","2. 拍摄光照质量","3. 目标尺寸","4. 目标完整性",
        "5. 数据均衡度","6. 产品丰富度","7. 目标密集度","8. 场景复杂度"
    ]:
        st.markdown(f"- {dim}")

st.header("📸 上传图片并生成多角度素材")
uploaded_file = st.file_uploader("上传一张无人机图片", type=['jpg','jpeg','png','bmp'])

if uploaded_file is not None:
    temp_dir = Path("temp_uploads")
    temp_dir.mkdir(exist_ok=True)
    temp_path = temp_dir / uploaded_file.name
    with open(temp_path, "wb") as f:
        f.write(uploaded_file.getbuffer())

    col1, col2 = st.columns([1,1])
    with col1:
        st.subheader("📷 原始图片")
        st.image(uploaded_file, caption="上传的原始图片", use_container_width=True)

    with col2:
        st.subheader("🎯 操作")
        if st.button("🚀 生成多角度素材并分析", type="primary", use_container_width=True):
            with st.spinner("正在生成多角度素材，请稍候..."):
                try:
                    output_dir = Path("generated_materials") / f"generation_{int(time.time())}"
                    output_dir.mkdir(parents=True, exist_ok=True)
                    progress_bar = st.progress(0)
                    status_text = st.empty()

                    status_text.text("步骤1/2: 正在生成多角度素材...")
                    result = st.session_state.generator.generate_multi_angle_images(
                        input_image_path=str(temp_path),
                        output_dir=str(output_dir),
                        num_generations=num_generations
                    )
                    progress_bar.progress(50)
                    status_text.text(f"✅ 已生成 {result['num_generated']} 张素材")
                    st.session_state.generated_images = result['generated_files']

                    if auto_analyze:
                        status_text.text("步骤2/2: 正在分析生成的素材...")
                        analysis_result = st.session_state.agent.analyze_and_evaluate(
                            result['generated_files']
                        )
                        st.session_state.analysis_results = analysis_result
                        progress_bar.progress(100)
                        status_text.text("✅ 分析完成！")
                    else:
                        progress_bar.progress(100)
                        status_text.text("✅ 生成完成！")

                    st.success(f"✅ 成功生成 {result['num_generated']} 张多角度素材！")
                except Exception as e:
                    st.error(f"❌ 处理出错: {e}")

    if st.session_state.generated_images:
        st.markdown("---")
        st.subheader("🎨 生成的多角度素材")
        num_cols = 4
        images = st.session_state.generated_images
        for i in range(0, len(images), num_cols):
            cols = st.columns(num_cols)
            for j, col in enumerate(cols):
                if i + j < len(images):
                    img_path = Path(images[i + j])
                    if img_path.exists():
                        col.image(Image.open(img_path), caption=img_path.name, use_container_width=True)

    if st.session_state.analysis_results:
        st.markdown("---")
        st.subheader("📊 8维度雷达图分析")
        avg_scores = st.session_state.analysis_results['analysis']['average_scores']
        fig = go.Figure()
        dimensions = [
            "图片数据量","拍摄光照质量","目标尺寸","目标完整性",
            "数据均衡度","产品丰富度","目标密集度","场景复杂度"
        ]
        values = [avg_scores.get(dim,0) for dim in dimensions]
        fig.add_trace(go.Scatterpolar(
            r=values + [values[0]],
            theta=dimensions + [dimensions[0]],
            fill='toself',
            name='维度得分',
            line=dict(color='rgb(31,119,180)', width=3),
            fillcolor='rgba(31,119,180,0.25)'
        ))
        avg_score = np.mean(values)
        fig.add_trace(go.Scatterpolar(
            r=[avg_score]*(len(dimensions)+1),
            theta=dimensions + [dimensions[0]],
            name=f'平均线 ({avg_score:.1f}%)',
            line=dict(color='rgb(255,127,14)', width=2, dash='dash')
        ))
        fig.update_layout(
            polar=dict(
                radialaxis=dict(visible=True, range=[0,100]),
                angularaxis=dict(rotation=90, direction='counterclockwise')
            ),
            showlegend=True,
            title="8维度质量分析雷达图",
            height=600
        )
        st.plotly_chart(fig, use_container_width=True)

        col1, col2 = st.columns([2,1])
        with col1:
            st.markdown("#### 📈 维度得分详情")
            score_df = pd.DataFrame([
                {"维度": dim, "得分": f"{score:.2f}%", "等级": "优秀 ⭐⭐⭐" if score>=90 else "良好 ⭐⭐" if score>=80 else "中等 ⭐" if score>=70 else "一般" if score>=60 else "较差"}
                for dim, score in avg_scores.items()
            ])
            st.dataframe(score_df, use_container_width=True, hide_index=True)
        with col2:
            st.markdown("#### 📊 统计信息")
            overall_quality = st.session_state.analysis_results['recommendations']['overall_quality']
            st.metric("整体质量", f"{overall_quality:.2f}%")
            st.metric("生成素材数", len(st.session_state.generated_images))
            st.metric("分析素材数", st.session_state.analysis_results['analysis']['total_images'])

        st.markdown("#### 📋 素材质量评估")
        quality_data = []
        for item in st.session_state.analysis_results['quality_evaluation']:
            quality_data.append({
                "素材": Path(item['image_path']).name,
                "平均得分": f"{item['average_score']:.2f}%",
                "质量等级": item['quality_level']
            })
        st.dataframe(pd.DataFrame(quality_data), use_container_width=True, hide_index=True)


if __name__ == "__main__":
    st.run()
'@ | Set-Content "app\web\material_generator_app.py" -Encoding UTF8
```

